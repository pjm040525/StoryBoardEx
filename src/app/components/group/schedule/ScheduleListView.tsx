...
export function ScheduleListView() {
  ...
  return (
    <div className="space-y-4 pb-20">
      {schedules.map((item) => (
        <Card key={item.id} className="border-stone-100 shadow-sm hover:shadow-md transition-shadow bg-white">
          <CardContent className="p-4">
            ...
            <div className="space-y-2 text-sm text-stone-600">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{item.attendees}명 참여/투표</span>
              </div>
            </div>
            
            <div className="mt-4 pt-3 border-t border-stone-100 flex justify-end">
              <Button size="sm" variant={item.status === 'voting' ? 'default' : 'outline'} className={item.status === 'voting' ? 'bg-orange-500 hover:bg-orange-600' : ''}>
